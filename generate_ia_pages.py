...
IA_CSS = """
    ...
    .ia-reprise { align-self:center; font-size:0.78rem; color:var(--muted); }
    .ia-reprise button { border:0; background:transparent; color:var(--primary); font-family:inherit; font-size:0.78rem; font-weight:700; cursor:pointer; text-decoration:underline; padding:6px; }
    ...
    @media (max-width: 768px) {
      ...
      .ia-chip { flex:0 0 auto; min-height:40px; }
      /* Les vingt boutons de reponse du selecteur mesuraient 32 px de haut.
         C'est le parcours en quatre questions : on les tape, il faut pouvoir
         les viser. */
      .ia-choice { min-height:40px; }
      /* Mesure sur 390 et 360 px : le texte des trois colonnes budgetaires et
         la mention de consentement tombaient a 12,5 px. Ce sont deux contenus
         qu'on lit vraiment — l'un pour choisir sa voie, l'autre parce qu'il
         engage. Le reste de la reponse etait deja a 14 px et ne bouge pas. */
      .ia-voie p { font-size:0.84rem; }
      .ia-recap-consent { font-size:0.84rem; }
...
{CTA}
  ...
  (function () {{
    ...
    // Reponse simple : ni outils ni questions. « Qui es-tu ? », ou une demande
    // hors sujet a laquelle il faut repondre sans faire semblant.
    function rendreMessage(el, r) {{
      stopAttente(el);
      el.innerHTML = '<span class="ia-conseil-source">FindIA</span><h3>' +
        echapper(r.situation) + '</h3>' +
        ((r.suivis && r.suivis.length)
          ? '<div class="ia-suivis"><span>Vous pouvez essayer</span>' +
            r.suivis.map(function (x) {{
              return '<button type="button" data-suivi="' + echapper(x) + '">' + echapper(x) + '</button>';
            }}).join('') + '</div>'
          : '');
    }}
    ...
    function rendre(el, r) {{
      ...
      // Trois façons de faire, avant le détail : le visiteur se situe sans
      // qu'on lui ait demandé son budget.
      function colonne(titre, sous, v, recommandee) {{
        if (!v || !v.outils || !v.outils.length) return '';
        return '<div class="ia-voie' + (recommandee ? ' est-recommandee' : '') + '">' +
          '<b>' + titre + '</b><em>' + sous + '</em>' +
          '<div class="ia-voie-outils">' + v.outils.map(function (o) {{
            return '<a href="' + o.url + '">' + echapper(o.nom) + '</a>';
          }}).join('') + '</div>' +
          '<p>' + echapper(v.phrase) + '</p></div>';
      }}
      var colonnes = '';
      if (r.gratuit || r.performance) {{
        colonnes = '<div class="ia-voies">' +
          colonne('Sans payer', 'gratuit ou offre gratuite', r.gratuit, false) +
          colonne('Recommandé', 'meilleur rapport qualité-prix', {{
            outils: r.etapes.map(function (e) {{ return {{ nom: e.nom, url: e.url }}; }}),
            phrase: 'Le meilleur équilibre entre ce que ça coûte et ce que ça vous fait gagner.'
          }}, true) +
          colonne('Performance', 'si le budget n’est pas le sujet', r.performance, false) +
          '</div>';
      }}

      el.innerHTML =
        '<span class="ia-conseil-source">FindIA · ' +
    ...
    // La conversation survit à un rechargement, à un aller-retour vers une
    // fiche outil, à une reprise le lendemain. Elle reste dans le navigateur
    // du visiteur : rien n'est envoyé nulle part.
    var CLE_FIL = 'findia-conversation';
    var journal = [];
    ...
    function restituer() {{
      var brut;
      try {{ brut = JSON.parse(localStorage.getItem(CLE_FIL) || 'null'); }} catch (e) {{ return; }}
      if (!brut || !brut.journal || !brut.journal.length) return;
      // Au-delà de sept jours, la situation du visiteur a changé : on repart à zéro.
      if (Date.now() - (brut.date || 0) > 7 * 24 * 3600 * 1000) {{
      ...
      journal.forEach(function (entree) {{
        ...
        el.className = 'ia-msg ia-msg-ia';
        fil.appendChild(el);
        if (entree.r.mode === 'questions') rendreQuestions(el, entree.r);
        else if (entree.r.mode === 'recommandation' || entree.r.etapes) rendre(el, entree.r);
        else rendreMessage(el, entree.r);
      }});
    ...
    function envoyer(texte) {{
      ...
      bulleVisiteur(texte);
      journal.push({{ type: 'moi', texte: texte }});
      ...
      fetch('/api/conseil', {{
        ...
        .then(function (data) {{
          ...
          if (!r) {{
            ...
            stopAttente(bulle);
            rendreMessage(bulle, r);
            historique.push({{ role: 'assistant', content: r.situation }});
            saisie.focus();
          ...
          if (r) journal.push({{ type: 'ia', r: r }});
          memoriser();
          if (!saisie.disabled) boutonEnvoyer.disabled = false;
          bulle.scrollIntoView({{ behavior: 'smooth', block: 'nearest' }});
        }});
    }}
