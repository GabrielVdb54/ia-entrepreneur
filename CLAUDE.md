...
- **`meilleures-ia.html` + `ia/` + `data/` are the AI directory ("Les meilleures IA du moment")** — the only generated, regenerable part of the site:
  ...
  - **The advisor (`/api/conseil`)** turns a described situation into a chain of 2-4 tools ("Taplio to write the post, ChatGPT Images for the visual"), each with its role and how to use it, then the most relevant IA-Entrepreneur offer. Two tiers: the page's **local engine** answers instantly from the cards already in the DOM (weighted keyword scoring + a per-role chain), then `/api/conseil` replaces it with a tailored answer. If the API is missing, down or rate-limited, the visitor keeps the local answer and sees no error — so the feature works with or without a key.
    ...
    - The directory publicly states that no link is paid. **Partnerships/commissions are a stated future intention, so treat this as a checklist, not a footnote.** Before a single paid placement goes live, every one of these must change, and the ranking rules must be restated:
      1. `ia/<slug>.html` × 129 — "Lien direct vers l'éditeur, sans affiliation ni parrainage." (generated by `generate_ia_pages.py`).
      2. `ia/meilleures-ia-<cat>.html` × 16 + the hub — "Tarifs indicatifs constatés en MM/AAAA. Aucun lien rémunéré." (same generator).
      3. `meilleures-ia.html` FAQ **and its FAQPage JSON-LD** — "Cet annuaire contient-il des liens sponsorisés ou affiliés ? Non, aucun… Aucun éditeur ne paie pour figurer dans cet annuaire, pour y être mieux classé ou pour en être retiré." A JSON-LD answer that contradicts the page is worse than no JSON-LD: Google treats it as deceptive markup.
      4. `politique-confidentialite.html` — no change needed for affiliation itself, but any tracking parameter added to editor links becomes a new processing to declare.
      5. `llms.txt` — the directory section says "Aucun lien rémunéré." This one matters more than its size suggests: it is what AI crawlers read, so a stale claim there propagates into AI answers about the site.
      6. `veille_annuaire.py` checks that editor URLs carry no referral parameter; it will start reporting every paid link as an anomaly.
      Beyond the wording: a site that ranks tools and is paid for placement is a comparison platform under French consumer law (art. L111-7 du Code de la consommation, "opérateurs de plateformes en ligne"), which requires disclosing the existence of a commercial relationship and whether it influences ranking. Get that reviewed by a professional before selling the first placement — and note it sits badly with the Qualiopi positioning if it is not visibly disclosed. The honest structure is a separate, clearly-labelled "partenaire" badge that never changes the editorial ranking, with the `note` (/5) staying independent.
  - **`api/recap.js` sends the answer by email and feeds the nurturing sequence.** After a recommendation, the visitor can have the plan emailed; a separate never-pre-ticked box opts into commercial content (art. 6.1.a RGPD — the consent wording itself is archived, art. 7.1). The client posts **only slugs and an offer key**: the server rebuilds names, prices and links from the catalogue, so no free text from a visitor can ever reach an email — otherwise the site would be an open spam relay on its own domain. Delivery and the sequence are delegated to n8n via `N8N_RECAP_WEBHOOK` (optional `N8N_RECAP_TOKEN` for a bearer header); without that variable the endpoint returns 503 and `/api/conseil` sets `recap: false`, so the page never displays the block — no promise we cannot keep. Emails land in `ia_entrepreneur_recaps`, a table **separate** from the question log, which must stay strictly anonymous. Both are insert-only: the anon key is public, so no SELECT policy exists and nobody can read the address list with it. Rate limit: 5 per IP per 10 minutes, plus a honeypot field and an Origin check.
    - The n8n workflow receives `{email, consentement, consentement_texte, consentement_date, outils[{slug,nom,usage,niveau,prix,url}], offre{cle,titre,url}, source, date}`. It must send the recap in every case, and only add to the nurturing list when `consentement === true`.
